...
def resize_np_array_to_512(img, num_channels=3):
    new_img = np.zeros((512,512,num_channels)).astype(np.float32)
    for row in range(256):
        for col in range(256):
            new_img[2*row:2*(row+1), 2*col:2*(col+1)] = img[row, col]
    return new_img
...
#             img, gt, scale = random_scale(img, gt, config.train_scale_array)
        ...
        if aug <= 8:
            crop = (random.randint(0, 256), random.randint(0, 256))
            p_img = resize_np_array_to_512(img[crop[0]:crop[0]+256, crop[1]:crop[1]+256], num_channels=3)

            if gt is not None:
                p_gt = resize_np_array_to_512(gt[crop[0]:crop[0]+256, crop[1]:crop[1]+256], num_channels=1)[:, :, 0]
            else:
                p_gt = None
        else:
            p_img = img
...
class VOC(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        datadir = workdir.split('/')[-1].split('.')[0]       # train or train_aug or val
        if 'train' in datadir or 'val' in datadir:
            datadir = 'train_aug2'
        ...
        if self.pseudo_label is True:
            img_path = os.path.join(self._img_path ,datadir, names)
            print(img_path)
        ...
        sp = names.strip()
        # if 'test' in datadir:
        #     img_path = os.path.join(self._img_path, datadir, "image",  "image%s"%sp)
        if 'test' not in datadir:
            if 'unlabeled' in self._train_source:
                img_path = os.path.join(self._img_path, datadir, "image",  sp)
            else:
                img_path = os.path.join(self._img_path, datadir, "image",  "images_input_labeled%s"%sp)

        if './' not in names:
            gt_path = os.path.join(self._gt_path , datadir, "label", "images_masks%s"%sp)
