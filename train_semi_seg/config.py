...
if os.getenv('volna') is not None:
    ...
    C.volna = '/data/yue/heidelberg_segmentation/TorchSemiSeg/' # the path to the data dir.

"""please config ROOT_dir and user when u first using"""
C.repo_name = 'TorchSemiSeg'
C.abs_dir = '/data/yue/heidelberg_segmentation/TorchSemiSeg/exp.voc/voc8.res50v3+.CPS+CutMix'
print("%s"%C.abs_dir)
...
if os.getenv('snapshot_dir'):
    ...
    C.snapshot_dir = osp.abspath(osp.join(C.log_dir, "snapshot_baseline"))
...
C.train_source = osp.join(C.dataset_path, "subset_train_aug2/train_aug_labeled_1-{}.txt".format(C.labeled_ratio))
C.unsup_source = osp.join(C.dataset_path, "subset_train_aug2/train_aug_unlabeled_1-{}.txt".format(C.labeled_ratio))
...
C.num_classes = 13
C.background = 0
C.image_mean = 0  # 0.485, 0.456, 0.406
C.image_std = 1
C.image_height = 512
C.image_width = 512
C.num_train_imgs = 2239
C.num_eval_imgs = 32
C.num_unsup_imgs = 13308     # unsupervised samples
